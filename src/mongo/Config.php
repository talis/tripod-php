...
class Config implements IConfigInstance
{
    ...
    {
        if (isset($this->viewSpecs[$storeName][$viewId]['to_data_source'])) {
            return $this->getMongoCollection(
    ...
    {
        if (isset($this->searchDocSpecs[$storeName][$searchDocumentId]['to_data_source'])) {
            return $this->getMongoCollection(
    ...
    {
        if (isset($this->tableSpecs[$storeName][$tableId]['to_data_source'])) {
            return $this->getMongoCollection(
    ...
    public function getCollectionsForTables($storeName, array $tables = [], $readPreference = ReadPreference::RP_PRIMARY_PREFERRED): array
    {
        ...
        foreach ($tables as $table) {
            if (isset($this->tableSpecs[$storeName][$table]['to_data_source'])) {
    ...
    public function getCollectionsForViews($storeName, array $views = [], $readPreference = ReadPreference::RP_PRIMARY_PREFERRED): array
    {
        ...
        foreach ($views as $view) {
            if (isset($this->viewSpecs[$storeName][$view]['to_data_source'])) {
    ...
    public function getCollectionsForSearch($storeName, array $searchSpecIds = [], $readPreference = ReadPreference::RP_PRIMARY_PREFERRED): array
    {
        ...
        foreach ($searchSpecIds as $searchSpec) {
            if (isset($this->searchDocSpecs[$storeName][$searchSpec]['to_data_source'])) {
        ...
        foreach ($this->databases as $storeName => $storeConfig) {
            ...
            if (isset($storeConfig['pods'])) {
                foreach ($storeConfig['pods'] as $podName => $podConfig) {
                    ...
                    $this->cardinality[$storeName][$podName] = isset($podConfig['cardinality']) ? $podConfig['cardinality'] : [];
